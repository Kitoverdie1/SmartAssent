...
def render_sidebar():
    with st.sidebar:
        ...
        # หน้าแรก (ไฟล์ app.py เอง)
        st.page_link("app.py", label="📌 ภาพรวม / แสดงจาก QR", icon="🏠")
...
# ==============================
# แสดงรายละเอียดจากพารามิเตอร์ ?code=
# ==============================
def render_asset_from_query():
    """
    ถ้า URL มี ?code=LAB-AS-001 → ดึงข้อมูลจาก Excel มาแสดง
    ถ้าไม่มีหรือหาไม่เจอ → return False เพื่อให้ไปแสดงหน้า “ภาพรวม” ตามปกติ
    """
    ...
    if not EXCEL_PATH.exists():
        ...
        return True  # มี code แต่เปิดไม่ได้ ก็ถือว่าแสดงผลแล้ว (แสดง error)
    ...
    st.info("หน้านี้คือโหมดอ่านข้อมูลจากการสแกน QR (แก้ไขได้ที่หน้า Dashboard/หน้าแก้ไขข้อมูล)")
...
- รองรับการรวมเป็น **ป้าย A4 3×8** สำหรับพิมพ์แปะที่ครุภัณฑ์
    ...
    st.info(
        "ถ้าต้องการลองสแกน QR ให้ใช้ลิงก์รูปแบบ: "
        "`https://...streamlit.app/?code=LAB-AS-001` "
        "โดยเปลี่ยนค่า code ให้ตรงกับรหัสเครื่องมือใน Excel"
    )
